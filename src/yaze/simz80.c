...
#define POP(x)   do {            \
   WORD y=GetBYTE_pp(SP);\
   x = (WORD) (y | (GetBYTE_pp(SP) << 8));      \
} while (0)

#define PUSH(x) do {            \
   mm_PutBYTE(SP, (uint8_t)((x) >> 8));                 \
   mm_PutBYTE(SP, (uint8_t)x);                        \
} while (0)
...
#define SET_FLAG_INC(temp)\
      AF = (uint32_t)((AF & (uint32_t)~0xfe) | (temp & 0xa8) |\
         (uint32_t)(((temp & 0xff) == 0) << 6) |\
         (uint32_t)(((temp & 0xf) == 0) << 4) |\
         (uint32_t)((temp == 0x80) << 2))

#define SET_FLAG_DEC(temp) \
      AF = (AF & (uint32_t)~0xfe) | (temp & 0xa8) |\
         (uint32_t)(((temp & 0xff) == 0) << 6) |\
         (uint32_t)(((temp & 0xf) == 0xf) << 4) |\
...
static void  dbg_reg_set(int which, uint32_t value) {
  switch (which) {
  case i_A:
    Sethreg(af[0], (WORD)value);
    break;
  case i_F:
    Setlreg(af[0], (WORD)value);
    break;
  case i_BC:
    regs[0].bc = (WORD)value;
    break;
  case i_DE:
    regs[0].de = (WORD)value;
    break;
  case i_HL:
    regs[0].hl = (WORD)value;
    break;
  case i_A_:
    Sethreg(af[1], (WORD)value);
    break;
  case i_F_:
    Setlreg(af[1], (WORD)value);
    break;
  case i_BC_:
    regs[1].bc = (WORD)value;
    break;
  case i_DE_:
    regs[1].de = (WORD)value;
    break;
  case i_HL_:
    regs[1].hl = (WORD)value;
    break;
  case i_IX:
    ix = (WORD)value;
    break;
...
static size_t dbg_reg_print(int which, char *buf, size_t bufsize) {
   if (which == i_F || which == i_F_) {
      ...
      return (size_t) snprintf(buf, bufsize, "%"PRIx32, dbg_reg_get(which));
   } else {
...
simz80_execute(int tube_cycles)
{
 ...
 do {
   ...
   case 0x0F:         /* RRCA */
      ...
      break;
   case 0x10:         /* DJNZ dd */
      PC += ((BC -= 0x100) & 0xff00) ? (WORD)((signed char) GetBYTE(PC) + 1) : 1;
      break;
   ...
   case 0x16:         /* LD D,nn */
      ...
      break;
   case 0x17:         /* RLA */
      AF = ((AF << 8) & 0x0100) | ((AF >> 7) & 0x28) | ((AF << 1) & ~0x01ffu) |
         (AF & 0xc4) | ((AF >> 15) & 1);
      break;
   case 0x18:         /* JR dd */
      PC += (1) ? (WORD)((signed char) GetBYTE(PC) + 1 ): 1;
      break;
   case 0x19:         /* ADD HL,DE */
      HL &= 0xffff;
      DE &= 0xffff;
   ...
   case 0x1F:         /* RRA */
      ...
      break;
   case 0x20:         /* JR NZ,dd */
      PC += (!TSTFLAG(Z)) ? (uint32_t)((signed char) GetBYTE(PC) + 1 ): 1;
      break;
   ...
   case 0x27:         /* DAA */
      ...
      cbits |= (acu >> 8) & 1;
      acu &= 0xff;
      AF = (acu << 8) | (acu & 0xa8) | (uint32_t)((acu == 0) << 6) |
   ...
   case 0x2E:         /* LD L,nn */
      ...
      break;
   case 0x2F:         /* CPL */
      AF = (~AF & ~0xffu) | (AF & 0xc5) | ((~AF >> 8) & 0x28) | 0x12;
      break;
   case 0x30:         /* JR NC,dd */
      PC += (!TSTFLAG(C)) ? (uint32_t)((signed char) GetBYTE(PC) + 1) : 1;
      break;
   ...
   case 0x3E:         /* LD A,nn */
      ...
      break;
   case 0x3F:         /* CCF */
      AF = (AF&~0x3bu)|((AF>>8)&0x28)|((AF&1)<<4)|(~AF&1);
      break;
   case 0x40:         /* LD B,B */
      /* nop */
      break;
   ...
   case 0xA0:         /* AND B */
      sum = ((AF & (BC)) >> 8) & 0xff;
      AF = (sum << 8) | (sum & 0xa8) |
         (uint32_t)((sum == 0) << 6) | 0x10 | partab[sum];
      break;
   case 0xA1:         /* AND C */
      sum = ((AF >> 8) & BC) & 0xff;
      AF = (sum << 8) | (sum & 0xa8) | 0x10 |
         (uint32_t)((sum == 0) << 6) | partab[sum];
      break;
   case 0xA2:         /* AND D */
      sum = ((AF & (DE)) >> 8) & 0xff;
      AF = (sum << 8) | (sum & 0xa8) |
   ...
   case 0xA8:         /* XOR B */
      sum = ((AF ^ (BC)) >> 8) & 0xff;
      AF = (sum << 8) | (sum & 0xa8) | ((uint32_t)(sum == 0) << 6) | partab[sum];
      break;
   case 0xA9:         /* XOR C */
      sum = ((AF >> 8) ^ BC) & 0xff;
      AF = (sum << 8) | (sum & 0xa8) | (uint32_t)((sum == 0) << 6) | partab[sum];
   ...
   case 0xB8:         /* CP B */
      temp = hreg(BC);
      AF = (AF & (uint32_t)~0x28) | (temp & 0x28);
      acu = hreg(AF);
      sum = acu - temp;
      cbits = acu ^ temp ^ sum;
      SET_FLAG_CP(temp,sum,cbits);
      break;
   case 0xB9:         /* CP C */
      temp = lreg(BC);
      AF = (AF & (uint32_t)~0x28) | (temp & 0x28);
   ...
   case 0xCB:         /* CB prefix */
      ...
      case 0x00:      /* shift/rotate */
         ...
         case 0x10:   /* RL */
            ...
            goto cbshflg1;
         case 0x18:   /* RR */
            temp = (acu >> 1) | (uint32_t)(TSTFLAG(C) << 7);
            cbits = acu & 1;
            goto cbshflg1;
         case 0x20:   /* SLA */
            temp = acu << 1;
      ...
      case 0x40:      /* BIT */
         if (acu & (1 << ((op >> 3) & 7)))
            AF = (AF & (uint32_t)~0xfe) | 0x10 |
            (uint32_t)(((op & 0x38) == 0x38) << 7);
         else
            AF = (AF & (uint32_t)~0xfe) | 0x54;
         if ((op&7) != 6)
            AF |= (acu & 0x28);
         temp = acu;
         break;
      case 0x80:      /* RES */
         temp = acu & (uint32_t)~(1 << ((op >> 3) & 7));
         break;
      case 0xc0:      /* SET */
         temp = acu | (1 << ((op >> 3) & 7));
         break;
   ...
   case 0xDD:         /* DD prefix */
      ...
      case 0x2E:         /* LD IXL,nn */
         ...
         break;
      case 0x34:         /* INC (IX+dd) */
         adr = IX + (uint32_t)((signed char) GetBYTE_pp(PC));
         temp = GetBYTE(adr)+1;
         PutBYTE(adr, temp);
         SET_FLAG_INC(temp);
         break;
      ...
      case 0xA6:         /* AND (IX+dd) */
         ...
         AF = (uint32_t)(sum << 8) | (sum & 0xa8) | 0x10 |
            (uint32_t)((sum == 0) << 6) | partab[sum];
         break;
      case 0xAC:         /* XOR IXH */
         sum = ((AF ^ (IX)) >> 8) & 0xff;
      ...
      case 0xCB:         /* CB prefix */
         ...
         case 0x00:      /* shift/rotate */
            ...
            default:
            case 0x00:   /* RLC */
               temp = (uint32_t)(acu << 1) | (acu >> 7);
               cbits = temp & 1;
               goto cbshflg2;
            case 0x08:   /* RRC */
               temp = (acu >> 1) | (uint32_t)(acu << 7);
               cbits = temp & 0x80;
               goto cbshflg2;
            case 0x10:   /* RL */
               temp = (uint32_t)(acu << 1) | TSTFLAG(C);
               cbits = acu & 0x80;
               goto cbshflg2;
            case 0x18:   /* RR */
               temp = (acu >> 1) | (uint32_t)(TSTFLAG(C) << 7);
   ...
   case 0xED:         /* ED prefix */
      ...
      case 0x42:         /* SBC HL,BC */
         ...
         cbits = (HL ^ BC ^ sum) >> 8;
         HL = sum;
         AF = (AF & (uint32_t)~0xff) | ((sum >> 8) & 0xa8) |
            (uint32_t)(((sum & 0xffff) == 0) << 6) |
            (((cbits >> 6) ^ (cbits >> 5)) & 4) |
      ...
      case 0x7C:
         temp = hreg(AF);
         AF = (-(AF & 0xff00) & 0xff00);
         AF |= ((AF >> 8) & 0xa8) | (uint32_t)(((AF & 0xff00) == 0) << 6) |
            (uint32_t)(((temp & 0x0f) != 0) << 4) | (uint32_t)((temp == 0x80) << 2) |
            2 | (temp?1:0);
         break;
      case 0x45:         /* RETN */
      ...
      case 0x6E:
         ...
         break;
      case 0x47:         /* LD I,A */
         ir = (WORD)((ir & 255) | (AF & (uint32_t)~255));
         break;
      case 0x48:         /* IN C,(C) */
         temp = Input(BC);
         Setlreg(BC, temp);
      ...
      case 0x76:
         ...
         break;
      case 0x57:         /* LD A,I */
         AF = (AF & 0x29) | (ir & (uint32_t)~255) | ((ir >> 8) & 0x80) | (uint32_t)(((ir & ~255) == 0) << 6) | ((IFF & 2) << 1);
         break;
      case 0x58:         /* IN E,(C) */
         temp = Input(BC);
         Setlreg(DE, temp);
      ...
      case 0x7E:
         ...
         break;
      case 0x5F:         /* LD A,R */
         AF = (AF & 0x29) | (uint32_t)((ir & 255) << 8) | (ir & 0x80) | (uint32_t)(((ir & 255) == 0) << 6) | (uint32_t)((IFF & 2) << 1);
          ir = (ir + 1) & 0xff;
         break;
      case 0x60:         /* IN H,(C) */
         temp = Input(BC);
      ...
      case 0x67:         /* RRD */
         ...
         AF = (acu << 8) | (acu & 0xa8) | (uint32_t)(((acu & 0xff) == 0) << 6) |
            partab[acu] | (AF & 1);
         break;
      case 0x68:         /* IN L,(C) */
         temp = Input(BC);
      ...
      case 0xA0:         /* LDI */
         ...
         PutBYTE_pp(DE, acu);
         acu += hreg(AF);
         AF = (AF & (uint32_t)~0x3e) | (acu & 8) | (uint32_t)((acu & 2) << 4) |
            (uint32_t)(((--BC & 0xffff) != 0) << 2);
         break;
      case 0xA1:         /* CPI */
         acu = hreg(AF);
         temp = GetBYTE_pp(HL);
         sum = acu - temp;
         cbits = acu ^ temp ^ sum;
         AF = (AF & (uint32_t)~0xfe) | (sum & 0x80) | ((sum & 0xff)?0:1<<6) |
            (uint32_t)(((sum - ((cbits&16)>>4))&2) << 4) | (cbits & 16) |
            ((sum - ((cbits >> 4) & 1)) & 8) |
            ((--BC & 0xffff)?(1<<2):0 ) | 2;
...
//         SETFLAG(Z, hreg(BC) == 0);
         temp = hreg(BC);
         AF = (AF & (uint32_t)~0xff) | (temp & 0xbb) |
            (uint32_t)(((temp & 0xff) == 0) << 6) |
            (uint32_t)((temp == 0x7f) << 2);   // Not exact, but close
         break;
      case 0xA3:         /* OUTI */
...
//         SETFLAG(Z, lreg(BC) == 0);
      ...
      case 0xB1:         /* CPIR */
         ...
         cbits = acu ^ temp ^ sum;
         AF = (AF & (uint32_t)~0xfe) | (sum & 0x80) | (uint32_t)(!(sum & 0xff) << 6) |
            (((sum - ((cbits&16)>>4))&2) << 4) |
...
//         SETFLAG(N, 1);
//         SETFLAG(Z, 1);
         AF = (AF & (uint32_t)~0xff) | 0x42;   // Not exact, but close
         break;
      case 0xB3:         /* OTIR */
         temp = hreg(BC);
         if (temp == 0) temp |= 0x100;
...
//         SETFLAG(Z, 1);
   ...
   case 0xFD:         /* FD prefix */
      ...
      case 0x2E:         /* LD IYL,nn */
         ...
         break;
      case 0x34:         /* INC (IY+dd) */
         adr = IY + (uint16_t)((signed char) GetBYTE_pp(PC));
         temp = GetBYTE(adr)+1;
         PutBYTE(adr, temp);
         SET_FLAG_INC(temp);
         break;
      ...
      case 0xBE:         /* CP (IY+dd) */
         ...
         break;
      case 0xCB:         /* CB prefix */
         adr = IY + (uint32_t)((signed char) GetBYTE_pp(PC));
         switch ((op = GetBYTE(PC)) & 7) {
             /*
              * default: suppresses compiler warning: "warning:
              * 'acu' may be used uninitialized in this function"
